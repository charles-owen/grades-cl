<?php
/** @file
 * Class that manages assignment grades.
 */
 ...
 *
 * An object of this type is assocated with each Assignment object
 * and has a collection of objects derived from Grade that 
 * represent the grading components for the assignment.
 */
class Grading {
	/** Constructor
	 * @param $assignment Assignment for this grading
	 */
	public function __construct(Assignment $assignment) {
		$this->assignment = $assignment;
		$course = $assignment->course;
		$this->grades = new Grades($course);
	}
		
	/** Add a grading category
	 * @param $grade Object derived from Grade */
	public function add(Grade $grade) { 
		$this->assignmentgrades[] = $grade; 
		return $grade;
	}
	...
	/** Add a manual grading category to an assignment 
	 * @param $points Number of points allocated to this category 
	 * @param $tag Tag associated with this category
	 * @param $name Name of the category */
	public function add_manual($points, $tag, $name) {
		return $this->add(new GradeManual($this, $points, $tag, $name));
	}
	...
	public function add_override() {
		return $this->add(new GradeOverride($this));
	}
	...
	private function ensure_category() {
		 ...
		 */
		if(count($this->assignmentgrades) === 0) {
    ...
    /**
     * Assignment for this grading object
     * @return Assignment Assignment this grading is for
     */
	...
	private $grades;			// Grades model object
	
	private $assignmentgrades = array();	// The grades for the assignment
